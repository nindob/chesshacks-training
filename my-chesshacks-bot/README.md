...
## Model Training (external repo)

Per ChessHacks rules we keep all training scripts, dataset tooling, and experiment code outside this deployment repo. The companion workspace now lives at `../training-suite` (or in its own GitHub repo) and owns:

- dataset streaming from Kaggle + Lichess files
- the PyTorch training loop + Modal GPU entrypoint
- helpers for decompressing `.zst` archives

Only the trained weights move back here. Small checkpoints (<100 MB) can be dropped into `src/weights/model.pt`; anything larger should be uploaded to remote storage (Hugging Face Hub, S3, Modal volume, etc.) and referenced via environment variables before startup:

- `CHESS_MODEL_PATH`: point to a local file on disk (e.g., after downloading from your storage bucket during deploy).
- `CHESS_MODEL_URL`: optionally point at a remote HTTP(S) URL. The bot will download it into `CHESS_MODEL_CACHE` (defaults to `./.model_cache`) and load from there.

This keeps the competition repo light and satisfies the “no training code in deployment” requirement.
