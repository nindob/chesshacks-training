...
class ChessAgent:
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        requested_device = self.config.device or ("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def _load_weights(self, model_path: Optional[str]):
        ...
        if url:
            ...
            path = model_path or os.environ.get("CHESS_MODEL_PATH") or os.path.join(
    ...
    def select_move(self, board: chess.Board, time_left_ms: Optional[int]) -> Tuple[chess.Move, Dict[chess.Move, float]]:
        time_left = time_left_ms if time_left_ms is not None else 1000
        move, policy = self.mcts.search(board, time_left)
        return move, policy

    def _evaluate_board(self, board: chess.Board) -> Tuple[Dict[chess.Move, float], float]:
